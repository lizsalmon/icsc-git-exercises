...
git init --initial-branch=main --bare -q
cd ..
...
cd dr_bohr_work
sed -i 's/BG_SCALE = 50/BG_SCALE = 42  # Updated from new sensor data/' analyse_higgs.py
git add .
...
sed -i 's/BG_SCALE = 50/BG_SCALE = 55  # My manual tweak/' analyse_higgs.py
sed -i 's/plt.hist(data, bins=100)/plt.hist(data, bins=150, color="green")/' analyse_higgs.py
git add .
